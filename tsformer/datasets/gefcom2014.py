...
def process_csv():
    ...
    """
    df = load_raw_dataset()
    df.to_csv(os.path.join(config['data'], 'GEFCom2014/Load/gefcom2014.csv'),
              index=False)
...
def add_exogenous_variables(df, one_hot=True):
    ...
    """
    X_temp = df[['w{}'.format(i)
                 for i in range(1, 26)]].values[1:]  # temperature values
    ...
    if one_hot:
        ...
        return X_temp, ex_feat.values[:, -4 -
                                      (ex_feat.shape[1] - df.shape[1]):][1:]
    else:
