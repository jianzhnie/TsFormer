...
parser.add_argument('--data_dir', type=str, required=False, default="/home/wenqi-ao/userdata/workdirs/data/power_load/house_power", help='Directory of data.')
parser.add_argument('--fill_nan', type=str, required=False, default="median", help='Method to fill nan values')
...
def process_csv(args):
    ...
    """
    df = pd.read_csv(os.path.join(args.data_dir, "household_power_consumption.txt"), sep = ";")
    ...
    df = impute_missing(df, method=args.fill_nan, values_col=TARGET, datetime_col=DATETIME)
    print(df.head(5))
    print(df.tail(5))
    print(df.info())
    df.to_csv(os.path.join(args.data_dir, 'UCI_household_power_consumption_synth.csv'), index=False)
